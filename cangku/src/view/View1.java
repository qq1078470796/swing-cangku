...
private void BGYViewJumpActionPerformed(java.awt.event.ActionEvent evt) {                                            
	...
	public void start(int types) throws SQLException, ClassNotFoundException{
		titles = new String[]{"零件号","零件名"};
		LingjianDao dao= new LingjianDao();
		records.removeAllElements();//初始化向量对象
		Vector<String> vtemp ;
		if(types==1){//添加一个新的零件
			Lingjian lj=new Lingjian();
			lj.setLjnum(BGYNum.getText());
			lj.setLjname(BGYName.getText());
			dao.add(lj);
			ArrayList<Lingjian> temp = dao.findAll();
			for(Lingjian s:temp){
				vtemp= new Vector<String>();
				vtemp.add(s.getLjnum());
				vtemp.add(s.getLjname());
				records.add(vtemp);
			}
			...
			lj.setLjnum(BGYNum.getText());
			lj.setLjname(BGYName.getText());
			System.out.println(lj.getLjname());
			dao.deleteByName(lj);
			ArrayList<Lingjian> temp = dao.findAll();
			for(Lingjian s:temp){
				vtemp= new Vector<String>();
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
            	...
            	OpenView.view1.setVisible(true);
            }
        });
    }
