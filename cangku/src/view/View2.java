...
public class View2 extends javax.swing.JFrame {
    ...
    private void BGYViewJumpActionPerformed(java.awt.event.ActionEvent evt) {                                            
    	...
    	OpenView.view.show();
    	OpenView.view2.setVisible(false);
    }   
    ...
    private void findOneActionPerformed(java.awt.event.ActionEvent evt) {                                           
    	try {
			start(6);
		} catch (ClassNotFoundException | SQLException e) {
			// TODO 自动生成的 catch 块
	...
	public void start(int types) throws SQLException, ClassNotFoundException{
		titles = new String[]{"零件号","仓库号","数量"};
		BaocunDao dao=new BaocunDao();
		records.removeAllElements();//初始化向量对象
		Vector<String> vtemp ;
		if(types==1){//查找所有
			ArrayList<Baocun> temp = dao.findAll();
			for(Baocun s:temp){
				vtemp= new Vector<String>();
				vtemp.add(s.getLjnum());
			...
			lj.setLjnum(BGYName.getText());
			bc.setLjsl(Integer.parseInt(BGYCKNum.getText()));
			dao.add(bgy,lj,bc.getLjsl());
			ArrayList<Baocun> temp = dao.findAll();
			for(Baocun s:temp){
				vtemp= new Vector<String>();
				vtemp.add(s.getLjnum());
			...
			Baoguanyuan bgy=new Baoguanyuan();
			bgy.setBaonum(BGYNum.getText());
			System.out.println(bgy.getBaonum());
			ArrayList<Baocun> temp = dao.findAllByBGY(bgy);
			for(Baocun s:temp){
				vtemp= new Vector<String>();
				vtemp.add(s.getLjnum());
				vtemp.add(s.getCknum());
		...
		else if(types==5){//通过零件查询该零在各个仓库中的数量
			Lingjian lj=new Lingjian();
			lj.setLjnum(BGYName.getText());
			ArrayList<Baocun> temp = dao.findAllByLJ(lj);
			for(Baocun s:temp){
				vtemp= new Vector<String>();
				vtemp.add(s.getLjnum());
				vtemp.add(s.getCknum());
    ...
    public static void main(String args[]) {
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
            	...
            	OpenView.view2.setVisible(true);
